...
async function sendAction(studentUid,type,payload,button,notify=true){
  if(!state.user)throw new Error("Koç oturumu gerekli");if(button)button.disabled=true;
  try{await setDoc(doc(collection(db,COLLECTIONS.actions)),{studentUid,coachUid:state.user.uid,type,payload,status:"pending",createdAt:serverTimestamp(),updatedAt:serverTimestamp()});if(notify)alert("Gönderildi ✓")}finally{if(button)button.disabled=false}
}
