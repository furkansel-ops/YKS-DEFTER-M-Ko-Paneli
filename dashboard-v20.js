(()=>{
  ...
  function programsPage(){
    ...
    const selected=selectedStudent(),week=weekInfo(selected),types=new Map();
    for(const s of students){const key=studentTrack(s);types.set(key,(types.get(key)||0)+1)}
    return`${pageHead("Programlar","Öğrencilerin çalışma programlarını görüntüle, takip et ve görev gönder.",`<button class="coach-primary" data-program-task>+ Programa Görev Gönder</button>`)}
      ...
      <section class="coach-grid equal"><article class="coach-card">${cardHeader("Program Türü Dağılımı","Öğrenci alanlarına göre")}<div class="coach-legend-list">${[...types.entries()].map(([type,count])=>`<div class="coach-legend-item"><i></i><span>${esc(type)}</span><b>${count} · %${students.length?Math.round(count/students.length*100):0}</b></div>`).join("")||empty("Program türü verisi yok.")}</div></article><article class="coach-card">${cardHeader("Son Program Güncellemeleri","Canlı eşitleme hareketleri")}${recentActivities(7)}</article></section>`;
  }
  ...
  function topicsPage(){
    ...
    return`${pageHead("Konular","Öğrencilerin konu ilerleyişini analiz et, eksikleri takip et ve çalışma planını yönlendir.")}
      ...
      <section class="coach-grid two"><article class="coach-card">${cardHeader("Konu Listesi","Tüm öğrencilerde paylaşılan konu kayıtları")}<div class="coach-table-wrap"><table class="coach-table"><thead><tr><th>Ders</th><th>Konu</th><th>Öğrenci</th><th>İlerleme</th><th>Son Tarih</th><th>Durum</th><th></th></tr></thead><tbody>${rows||`<tr><td colspan="7">${empty("Konu verisi yok.")}</td></tr>`}</tbody></table></div></article><aside class="coach-grid"><article class="coach-card">${cardHeader("Seçili Konu Özeti")}${selected?`<div class="coach-selected-card"><div class="coach-selected-head"><span class="coach-avatar">${esc(initials(studentName(selected.student)))}</span><div><h3>${esc(selected.topic.topic||selected.topic.name||"Konu")}</h3><p>${esc(selected.topic.subject||"Ders")} · ${esc(studentName(selected.student))}</p></div><span class="coach-status ${num(selected.topic.st)>=3?"":num(selected.topic.st)>0?"info":"warn"}">${num(selected.topic.st)>=3?"Tamamlandı":num(selected.topic.st)>0?"Devam":"Başlanmadı"}</span></div><div class="coach-selected-metrics"><div class="coach-selected-metric"><span>Aşama</span><b>${num(selected.topic.st)}/3</b></div><div class="coach-selected-metric"><span>Son tarih</span><b>${esc(selected.topic.deadline||"—")}</b></div><div class="coach-selected-metric"><span>Alan</span><b>${esc(selected.topic.exam||studentTrack(selected.student))}</b></div></div><div class="coach-quick-actions"><button class="coach-quick-btn" data-student-detail="${esc(selected.student.studentUid)}" data-detail-tab="topics">Konu detayını aç <span>›</span></button><button class="coach-quick-btn" data-student-detail="${esc(selected.student.studentUid)}" data-detail-tab="program">Programa git <span>›</span></button></div></div>`:empty("Konu seçimi için veri bekleniyor.")}</article><article class="coach-card">${cardHeader("Ders Bazlı Konu İlerlemesi")}${subjectList(8)}</article></aside></section>
      <section class="coach-grid equal"><article class="coach-card">${cardHeader("Konu Durumu Dağılımı")}<div class="coach-donut-wrap"><div class="coach-donut"><strong>${flat.length}<small>Konu</small></strong></div><div class="coach-legend-list"><div class="coach-legend-item"><i></i><span>Tamamlanan</span><b>${complete}</b></div><div class="coach-legend-item"><i></i><span>Devam eden</span><b>${active}</b></div><div class="coach-legend-item"><i></i><span>Geciken</span><b>${overdue}</b></div></div></div></article><article class="coach-card">${cardHeader("Son Konu Hareketleri","Öğrenci paylaşım güncellemeleri")}${recentActivities(7)}</article></section>`;
  }
  ...
  function errorsPage(){
    ...
    return`${pageHead("Hata Defteri","Öğrencilerin yaptıkları hataları takip et, tekrar eden konuları analiz et ve gelişimi destekle.")}
      ...
      <section class="coach-grid two"><article class="coach-card">${cardHeader("Hata Kayıtları","Öğrencilerin paylaşılan hata defterleri")}<div class="coach-table-wrap"><table class="coach-table"><thead><tr><th>Ders</th><th>Konu</th><th>Öğrenci</th><th>Hata Türü</th><th>Tekrar</th><th>Durum</th><th></th></tr></thead><tbody>${rows||`<tr><td colspan="7">${empty("Hata kaydı yok.")}</td></tr>`}</tbody></table></div></article><aside class="coach-grid"><article class="coach-card">${cardHeader("Seçili Hata Özeti")}${selected?`<div class="coach-selected-card"><div class="coach-selected-head"><span class="coach-avatar">${esc(initials(studentName(selected.student)))}</span><div><h3>${esc(selected.error.topic||selected.error.konu||"Hata Kaydı")}</h3><p>${esc(selected.error.subject||selected.error.ders||"Ders")} · ${esc(studentName(selected.student))}</p></div><span class="coach-status ${selected.count>=4?"risk":"warn"}">${selected.count>=4?"Kritik":"Açık"}</span></div><div class="coach-selected-metrics"><div class="coach-selected-metric"><span>Tekrar</span><b>${selected.count}</b></div><div class="coach-selected-metric"><span>Hata türü</span><b>${esc(selected.error.type||selected.error.errorType||"Hata")}</b></div><div class="coach-selected-metric"><span>Ders</span><b>${esc(selected.error.subject||selected.error.ders||"—")}</b></div></div><div class="coach-quick-actions"><button class="coach-quick-btn" data-student-detail="${esc(selected.student.studentUid)}" data-detail-tab="errors">Hata defterini aç <span>›</span></button><button class="coach-quick-btn" data-message-student="${esc(selected.student.studentUid)}">Öğrenciye not gönder <span>›</span></button></div></div>`:empty("Hata kaydı seçimi için veri yok.")}</article><article class="coach-card">${cardHeader("Ders Bazlı Hata Yoğunluğu")}${subjects.length?`<div class="coach-subject-list">${subjects.map(([name,count])=>`<div class="coach-subject-row"><span>${esc(name)}</span><div class="coach-subject-track"><i style="width:${Math.min(100,total?count/total*100:0)}%"></i></div><b>${count}</b></div>`).join("")}</div>`:empty("Hata verisi yok.")}</article></aside></section>
      <section class="coach-grid equal"><article class="coach-card">${cardHeader("Hata Türü Dağılımı")}<div class="coach-donut-wrap"><div class="coach-donut"><strong>${total}<small>Toplam Hata</small></strong></div><div class="coach-legend-list">${subjects.slice(0,4).map(([name,count])=>`<div class="coach-legend-item"><i></i><span>${esc(name)}</span><b>${count}</b></div>`).join("")||empty("Dağılım verisi yok.")}</div></div></article><article class="coach-card">${cardHeader("Son Hata Hareketleri","Canlı paylaşım güncellemeleri")}${recentActivities(7)}</article></section>`;
  }
